...
const fromBuf = (b) => b == null ? 0 : asBuf(b).readInt32LE();
const toBuf = (n) => {
...
const conflictWrites = async () => {
    ...
    let txnAttempts = 0;
    await Promise.all(new Array(100).fill(0).map((_, i) => (db.doTransaction(async (tn) => {
...
const batchToStr = (batch) => batch.map(([k, v]) => [k.toString(), v.toString()]);
...
const rangeTest2 = async () => {
    ...
    console.log(batchToStr(await db.getRangeAll(fdb.keySelector.firstGreaterThan('a'), fdb.keySelector.firstGreaterThan('c')))); // 'b'
};
const rangeTest3 = async () => {
    ...
    await db.getRangeAll('c', 'b', {
        // limit: 0,
        streamingMode: opts_g_1.StreamingMode.Medium,
        limit: 397,
        reverse: false,
    });
};
...
const crash = async () => {
    ...
    watch.promise.then(v => console.log('listener called', v), err => console.error('listener err', err));
