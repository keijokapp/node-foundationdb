    ...
    */
    ...
    _encodeRangeResult(r) {
        ...
        for (let i = 0; i < r.length; i++) {
            ;
            r[i][0] = this.subspace.unpackKey(r[i][0]);
            r[i][1] = this.subspace.unpackValue(r[i][1]);
        }
        return r;
    }
    ...
    limit, targetBytes, streamingMode, iter, reverse) {
        this._assertValid();
        const _end = end != null ? end : keySelector_1.default.firstGreaterOrEqual((0, util_1.strInc)(start.key));
    ...
    getRangeRaw(start, end, limit, targetBytes, streamingMode, iter, reverse) {
        return this.getRangeNative((0, keySelector_1.default)(this.subspace.packKey(start.key), start.orEqual, start.offset), end != null ? (0, keySelector_1.default)(this.subspace.packKey(end.key), end.orEqual, end.offset) : null, limit, targetBytes, streamingMode, iter, reverse)
    ...
    getRangeSplitPoints(start, end, chunkSize) {
        this._assertValid();
        const range = this.subspace.packRange(start, end, true);
        return this._tn.getRangeSplitPoints(range.begin, range.end, chunkSize).then(results => (results.map(r => this.subspace.unpackKey(r))));
    }
    async *getRangeBatchNative(start, end, { limit = 0, reverse = false, streamingMode = opts_g_1.StreamingMode.Iterator } = {}) {
        let iter = 0;
        while (1) {
            const { results, more } = await this.getRangeNative(start, end, limit, 0, streamingMode, ++iter, reverse);
            if (results.length) {
                if (!reverse)
    ...
    getRangeBatch(start, end, opts = {}) {
        const startSelector = keySelector_1.default.from(start);
        const endSelector = keySelector_1.default.from(end);
        const range = this.subspace.packRange(startSelector.key, endSelector.key);
        return this.getRangeBatchNative((0, keySelector_1.default)(range.begin, startSelector.orEqual, startSelector.offset), (0, keySelector_1.default)(range.end, startSelector.orEqual, startSelector.offset), opts);
    }
    /**
    ...
    getRangeBatchStartsWith(prefix, opts) {
        const prefixSelector = keySelector_1.default.from(prefix);
        const range = this.subspace.packRangeStartsWith(prefixSelector.key);
        return this.getRangeBatchNative((0, keySelector_1.default)(range.begin, prefixSelector.orEqual, prefixSelector.offset), keySelector_1.default.firstGreaterOrEqual(range.end), opts);
    }
    /**
    ...
    async getRangeAll(start, end, opts) {
        const childOpts = opts?.streamingMode == null
            ? { ...opts, streamingMode: opts_g_1.StreamingMode.WantAll }
            : opts;
        const result = [];
    ...
    addReadConflictKey(key) {
        this._assertValid();
        const keyBuf = this.subspace.packKey(key);
        this._tn.addReadConflictRange(keyBuf, (0, util_1.strNext)(keyBuf));
    }
    ...
    addWriteConflictKey(key) {
        this._assertValid();
        const keyBuf = this.subspace.packKey(key);
        this._tn.addWriteConflictRange(keyBuf, (0, util_1.strNext)(keyBuf));
    }
    ...
    setVersionstampedKeyRaw(keyBytes, value) {
        this.atomicOpNative(opts_g_1.MutationType.SetVersionstampedKey, keyBytes, this.subspace.packValue(value));
    }
    // This sets the key [prefix, 10 bytes versionstamp, suffix] to value.
    setVersionstampedKeyBuf(prefix, suffix, value) {
        const key = (0, versionstamp_1.packVersionstampPrefixSuffix)(prefix, suffix, true);
        // console.log('key', key)
        this.atomicOpNative(opts_g_1.MutationType.SetVersionstampedKey, key, this.subspace.packValue(value));
    }
    ...
    setVersionstampedKey(key, value, bakeAfterCommit = true) {
        ...
        this.setVersionstampedKeyRaw((0, versionstamp_1.packVersionstamp)(pack, true), value);
        if (bakeAfterCommit)
            this._addBakeItem(key, this.subspace._bakedKeyXf, code);
    }
    setVersionstampSuffixedKey(key, value, suffix) {
        const prefix = (0, util_1.asBuf)(this.subspace.packKey(key));
    ...
    setVersionstampedValue(key, value, bakeAfterCommit = true) {
        ...
        const code = this._bakeCode(pack);
        const val = (0, versionstamp_1.packVersionstamp)(pack, false);
    ...
    setVersionstampPrefixedValue(key, value, prefix) {
        const valBuf = value !== undefined ? (0, util_1.asBuf)(this.subspace.packValue(value)) : undefined;
        const val = (0, versionstamp_1.packVersionstampPrefixSuffix)(prefix, valBuf, false);
        this.atomicOpKB(opts_g_1.MutationType.SetVersionstampedValue, key, val);
    }
    /**
