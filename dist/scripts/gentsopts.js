...
const fs = require("fs");
const xml2js = require("xml2js"); // I could type this but its not important enough.
...
`);
const toLowerCamelCase = (str) => str.replace(/(^\w|_\w)/g, c => c.length == 1 ? c : c[1].toUpperCase());
const toUpperCamelCase = (str) => str.replace(/(^\w|_\w)/g, c => c.length == 1 ? c.toUpperCase() : c[1].toUpperCase());
    ...
    result.Options.Scope.forEach((scope) => {
        ...
        if (name.endsWith('Option')) {
            ...
            options.forEach(({ name, type, paramDescription, deprecated }) => {
                ...
                if (deprecated)
                    output.write(` ${comment} DEPRECATED`);
                else if (paramDescription)
                    output.write(`  ${comment} ${paramDescription}`);
                line();
            });
            line(`}\n`);
            enumName = name + 'Code';
...
//# sourceMappingURL=gentsopts.js.map