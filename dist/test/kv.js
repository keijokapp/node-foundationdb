...
(0, util_1.withEachDb)(db => describe('key value functionality', () => {
    ...
    it('retries conflicts', async function () {
        ...
        const key = 'num';
        await db.set(key, (0, util_1.numToBuf)(0));
        let txnAttempts = 0;
        await Promise.all(new Array(concurrentWrites).fill(0).map((_, i) => (db.doTransaction(async (tn) => {
            const val = (0, util_1.bufToNum)((await tn.get(key)));
            tn.set(key, (0, util_1.numToBuf)(val + 1));
            txnAttempts++;
        }))));
        const result = (0, util_1.bufToNum)((await db.get(key)));
    ...
    describe('native encoding', () => {
        ...
        const setGetAssertEqual = async (val, valueEncoding) => {
            await db.withKeyEncoding(lib_1.encoders.string).withValueEncoding(valueEncoding).doTransaction(async (tn) => {
                tn.set('xxx', val);
                const result = await tn.get('xxx');
                assert.deepStrictEqual(result, val);
            });
        ...
        for (const jsonStringifyLength of [1023, 1024, 1025]) {
            it(`handles ${jsonStringifyLength} length json`, async () => {
                await setGetAssertEqual(Array.from({ length: jsonStringifyLength - 2 }, (_, x) => (x % 10) + '').join(''), lib_1.encoders.json);
            });
        }
    });
    describe('getKey', () => {
        ...
        it('returns the key that matches if a key selector is passed', async () => {
            await db.set('a', 'y');
            await db.set('b', 'y');
            assert.strictEqual((await db.getKey(lib_1.keySelector.firstGreaterThan('a')))?.toString(), 'b');
        });
        it('returns undefined if the key selector matches outside of the subspace range', async () => {
            await db.at('b').set('key', 'val');
            const _db = db.at('a');
            assert.strictEqual(await _db.getKey((0, lib_1.keySelector)('', true, 0)), undefined);
        });
    });
    describe('version stamps', () => {
        ...
        it('handles setVersionstampedValue', async () => {
            const db_ = db.withValueEncoding(util_1.strXF);
        ...
        it('roundtrips a tuple key', async () => {
            const db_ = db.withKeyEncoding(lib_1.encoders.tuple);
        ...
        it('commits a tuple with unbound key versionstamps and bakes the vs and code', async () => {
            const db_ = db.withKeyEncoding(lib_1.encoders.tuple).withValueEncoding(lib_1.encoders.string);
            const key1 = [1, 2, 3, lib_1.tuple.unboundVersionstamp()]; // should end up with code 0
            const key2 = [1, 2, 3, lib_1.tuple.unboundVersionstamp()]; // code 1
            const key3 = [1, 2, 3, lib_1.tuple.unboundVersionstamp(321)]; // code 321
        ...
        it('bakes versionstamps in a subspace', async () => {
            const subspace = db.withValueEncoding(lib_1.encoders.tuple);
            const value = await db.doTransaction(async (tn) => {
                const value = [lib_1.tuple.unboundVersionstamp()];
                tn.scopedTo(subspace).setVersionstampedValue('some-key', value);
                return value;
            });
            assert.strictEqual(value[0].type, 'versionstamp');
        });
        ...
        it('commits a tuple with unbound value versionstamps and bakes the vs and code', async () => {
            const db_ = db.withKeyEncoding(lib_1.encoders.string).withValueEncoding(lib_1.encoders.tuple);
    ...
    describe('regression', () => {
        it('does not trim off the end of a string', async () => {
            ...
            await _db.set(util_1.prefix + 'somekey', 'val');
            const val = await _db.get(util_1.prefix + 'somekey');
