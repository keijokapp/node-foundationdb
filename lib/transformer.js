...
import {asBuf, concat2, strInc, startsWith, id} from './util.js'

/** @type {import('foundationdb').Transformer<Buffer | string, Buffer>} */
 ...
 * @param {import('foundationdb').Transformer<In, Out>} inner
 * @returns {import('foundationdb').Transformer<In, Out>}
 */
export const prefixTransformer = (prefix, inner) => {
  const _prefix = asBuf(prefix)
  /** @type {import('foundationdb').Transformer<In, Out>} */
  const transformer = {
  ...
  if (inner.packUnboundVersionstamp) transformer.packUnboundVersionstamp = (val) => {
    ...
    /** @type {import('foundationdb').UnboundStamp} */
    const unboundStamp = {
