 ...
 * @import { KeySelector, KVList, NativeValue, RangeOptions, Transformer, UnboundStamp, Version, Watch, WatchOptions } from './types'
 */
...
export default class Transaction {
  ...
  getKey(_sel) {
    ...
    return this._tn.getKey(this.subspace.packKey(sel.key), sel.orEqual, sel.offset, this.isSnapshot).then(
  ...
  getRangeNative(start, end, limit, targetBytes, streamingMode, iter, reverse) {
    ...
    const startKey = asBuf(start.key)
    const _end = end != null ? { ...end, key: asBuf(end.key) } : keySelector.firstGreaterOrEqual(strInc(startKey))

    return this._tn.getRange(startKey, start.orEqual, start.offset, _end.key, _end.orEqual, _end.offset, limit, targetBytes, streamingMode, iter, this.isSnapshot, reverse)
  }
   ...
   * transaction.
   *
   * @returns {{ promise: Promise<Version> }}
   */
  getVersionstamp() {
    this._assertValid()
  ...
  atomicOpNative(opType, key, oper) {
    this._assertValid()
    this._tn.atomicOp(opType, asBuf(key), asBuf(oper))
  }
   ...
   *
   * @param {KeyIn} key
   * @returns {Promise<{ stamp: Version, value?: ValOut } | undefined>}
   */
