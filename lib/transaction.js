 ...
 * @import { BaseTransaction, KeySelector, KVList, NativeValue, RangeOptions, Transformer, UnboundStamp, Version, Watch, WatchOptions } from './types.js'
 */
...
export default class Transaction {
   ...
   * committing (and retrying on conflict) exactly like `Database.doTn`.
   *
   * @template T
   * @param {(tn: Transaction<KeyIn, KeyOut, ValIn, ValOut>) => Promise<T>} body
   * @returns {Promise<T>}
  ...
  async doTn(body) {
    ...
    for (;;) {
      ...
      transaction = new Transaction(this._tn.createTransaction(), false, this.subspace)

      try {
        return await transaction._exec(body)
      } catch (err) {
        if (err instanceof FDBError) {
          await transaction._tn.onError(err.code)
        } else {
