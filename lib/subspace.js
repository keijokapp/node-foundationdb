...
import { identity } from './encoders.js'
import { prefixTransformer, defaultGetRange } from './transformer.js'
import {
...
export default class Subspace {
  ...
  constructor(rawPrefix, keyXf, valueXf) {
    ...
    this.keyXf = keyXf ?? /** @type {Transformer<any, any>} */(identity)
    /** @type {Transformer<ValIn, ValOut>} */
    this.valueXf = valueXf ?? /** @type {Transformer<any, any>} */(identity)
   ...
   * @template CKI, CKO
   * @overload
   * @param {NativeValue | undefined} prefix
   * @param {Transformer<CKI, CKO>} keyXf
   * @param {undefined} [valueXf]
   * @returns {Subspace<CKI, CKO, ValIn, ValOut>}
   */
  ...
  * @returns {Subspace<any, any, any, any>}
  */
  atRaw(prefix, keyXf = this.keyXf, valueXf = this.valueXf) {
    return new Subspace(concatPrefix(this.prefix, prefix), keyXf, valueXf)
  }
...
export const root = new Subspace()
