...
export default class Subspace {
   ...
   */
  packKey(key) {
    return asBuf(this._bakedKeyXf.pack(key))
  }
   ...
   */
  packValue(val) {
    return asBuf(this.valueXf.pack(val))
  }
   ...
   * @returns {{ begin: Buffer, end: Buffer }} Encoded range as a `{ begin, end }` record.
   */
  packRange(start, end) {
    return {
      begin: start !== undefined ? asBuf(this._bakedKeyXf.pack(start)) : this.prefix,
      end: end !== undefined ? asBuf(this._bakedKeyXf.pack(end)) : strInc(this.prefix)
    }
  }
  ...
  packRangeStartsWith(prefix) {
    if (this._bakedKeyXf.range != null) {
      const { begin, end } = this._bakedKeyXf.range(prefix)
    ...
    return defaultGetRange(prefix, this._bakedKeyXf)
  }
