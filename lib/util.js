...
export const strInc = val => {
  // TODO: keep temporarily for compatibility
  const buf = asBuf(val)
 ...
 * @returns {Buffer}
 */
export const strNext = val => Buffer.concat([val, byteZero], val.length + 1)

/**
 * @param {NativeValue} val
 * @returns {Buffer}
 */
export const asBuf = val => typeof val === 'string' ? Buffer.from(val) : val
 ...
 * @returns {boolean}
 */
export const startsWith = (a, prefix) => prefix.length <= a.length && prefix.compare(a, 0, prefix.length) === 0
