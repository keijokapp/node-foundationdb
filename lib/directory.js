...
import { biguint64LE, identity, tuple } from './encoders.js'
import { TransactionOptionCode } from './opts.g.js'
import Subspace, { root } from './subspace.js'
import {
  asBuf,
  concat2,
  emptyBuffer,
 ...
 * @import { DirectoryLayerOpts, NativeValue, Transformer } from './types.js'
 */
...
/**
 * @typedef {Database<any, any, any, any>} DbAny
 * @typedef {Transaction<any, any, any, any>} TxnAny
 * @typedef {Subspace<any, any, any, any>} SubspaceAny
 */
/**
 * @template KeyIn, KeyOut, ValIn, ValOut
 * @typedef {Database<KeyIn, KeyOut, ValIn, ValOut> | Transaction<KeyIn, KeyOut, ValIn, ValOut>} DbOrTxn
 */
/**
 * @typedef {undefined | TupleItem | TupleItem[]} TupleIn
 * Node subspaces have tuple keys like [SUBDIRS, (bytes)] and [b'layer'].
 * @typedef {Subspace<TupleIn, TupleItem[], NativeValue, Buffer>} NodeSubspace
 * @typedef {[number, number, number]} Version
 * @typedef {string | string[] | null | undefined} PathIn
 * @typedef {string[]} Path
 */
 ...
 *
 * @template KeyIn, KeyOut, ValIn, ValOut, T
 * @param {Database<KeyIn, KeyOut, ValIn, ValOut> | Transaction<KeyIn, KeyOut, ValIn, ValOut>} dbOrTxn
 * @param {(tn: Transaction<KeyIn, KeyOut, ValIn, ValOut>) => Promise<T>} body
 * @returns {Promise<T>}
 */
const doTxn = (dbOrTxn, body) => dbOrTxn instanceof Database ? dbOrTxn.doTn(body) : body(dbOrTxn)

/** @type {Transformer<void, void>} */
const voidEncoding = {
  pack() { return emptyBuffer },
  unpack() { return null },
}

/** @type {Transformer<Version, Version>} */
 ...
 * @template T
 * @param {TxnAny} tn
 * @param {() => Promise<Awaited<T>>} block
 * @param {Promise<Awaited<T>>} block
 */
function synchronized(tn, block) {
...
export class HighContentionAllocator {
  /**
   * @param {SubspaceAny} subspace
   */
  constructor(subspace) {
    /** @type {Subspace<undefined | number | [number?], [number], bigint, bigint>} */
    this.counters = subspace.withKeyEncoding(/** @type {Transformer<any, any>} */(tuple)).at(0).withValueEncoding(biguint64LE)
    /** @type {Subspace<TupleIn, TupleItem[], void, void>} */
  ...
  async allocate(tn) {
    ...
    const counters = tn.at(this.counters)
    // Recent marks all allocations inside the current window.
    const recent = tn.at(this.recent)

    return synchronized(tn, async () => {
      // eslint-disable-next-line prefer-const
      let [[[start], count] = [[0], 0n]] = await counters.snapshot().getRangeAll(undefined, undefined, { limit: 1, reverse: true })

      let window = windowSize(start)

      if ((count + 1n) * 2n >= window) {
        counters.clearRange(undefined, start)
        recent.setOption(TransactionOptionCode.NextWriteNoWriteConflictRange)
        recent.clearRange(undefined, start)
      ...
      for (;;) {
        ...
        const candidateInUse = await recent.exists(candidate)
 ...
 * eslint-disable-next-line no-nested-ternary
 *
 * @param {PathIn | null | undefined} path
 * @returns {Path}
 */
// eslint-disable-next-line no-nested-ternary
const normalizePath = path => path == null
  ? []
  : Array.isArray(path)
    ? path
    : [path]
...
class Node {
   ...
   *
   * Filled in lazily. Careful - this will most often be empty.
   *
  ...
  /**
   *
   * @param {NodeSubspace | undefined} subspace
   * @param {Path} path
   * @param {Path} targetPath
   */
  constructor(subspace, path, targetPath) {
    /** @type {NodeSubspace | undefined} */
  ...
  async getLayer(txn) {
    if (this.layer == null) {
      ...
      // 'partition'.
      if (txn) {
        this.layer = await txn.at(/** @type {NodeSubspace} */(this.subspace)).get(LAYER_KEY) ?? emptyBuffer
      } else {
  ...
  /**
   *
   * @param {boolean} [includeEmptySubpath]
   * @returns {boolean}
   */
  ...
  /**
   * @template KeyIn, KeyOut, ValIn, ValOut
   * @param {DirectoryLayer} directoryLayer
   * @param {Transformer<KeyIn, KeyOut>} keyXf
   * @param {Transformer<ValIn, ValOut>} valueXf
   * @param {TxnAny} [txn]
   * @returns {Promise<Directory<KeyIn, KeyOut, ValIn, ValOut> | undefined>}
   */
  ...
  /**
   * @param {DirectoryLayer} directoryLayer
   * @param {Transformer<any, any>} keyXf
   * @param {Transformer<any, any>} valueXf
   * @returns {Directory<any, any, any, any> | undefined}
   */
  getContentsSync(directoryLayer, keyXf = identity, valueXf = identity) {
    if (this.layer == null) {
 ...
 *
 * @template [KeyIn=NativeValue], [KeyOut=Buffer], [ValIn=NativeValue], [ValOut=Buffer]
 */
export class Directory {
  /**
   * @param {DirectoryLayer} parentDirectoryLayer
   * @param {Path} path
   * @param {Subspace<KeyIn, KeyOut, ValIn, ValOut>} contentSubspace
   * @param {boolean} isPartition
   * @param {NativeValue} [layer]
   */
  constructor(parentDirectoryLayer, path, contentSubspace, isPartition, layer) {
    ...
    if (isPartition) {
      ...
      const directoryLayer = new DirectoryLayer({
        nodeSubspace: contentSubspace.atRaw(DEFAULT_NODE_PREFIX, tuple, identity),
        contentSubspace,
      })
  ...
  /**
   * @param {TxnAny | DbAny} txnOrDb
   * @param {PathIn} path
   * @param {'partition' | NativeValue} [layer]
   * @returns {Promise<Directory<any, any, any, any>>}
   */
  createOrOpen(txnOrDb, path, layer) { // partition specified for autocomplete.
  ...
  /**
   * @param {TxnAny | DbAny} txnOrDb
   * @param {PathIn} oldPath
   * @param {PathIn} newPath
   * @returns {Promise<Directory<any, any, any, any>>}
   */
  ...
  /**
   * @param {PathIn} path
   * @returns {DirectoryLayer}
   */
  _getLayerForPath(path) {
    // eslint-disable-next-line no-nested-ternary
    return this.isPartition()
      ? normalizePath(path).length === 0
        ? /** @type {DirectoryLayer} */(this._parentDirectoryLayer)
...
export class DirectoryLayer {
  ...
  constructor(opts = {}) {
    ...
    this._nodeSubspace = opts.nodeSubspace != null
      ...
      : new Subspace(opts.nodePrefix ?? DEFAULT_NODE_PREFIX, tuple)

    /** @type {Subspace<TupleIn, TupleItem[], any, any>} */
    this._contentSubspace = opts.contentSubspace != null
      ? opts.contentSubspace.withKeyEncoding(tuple)
      : new Subspace(opts.contentPrefix, tuple)
   ...
   *
   * @template KeyIn, KeyOut, ValIn, ValOut
   * @param {DbOrTxn<KeyIn, KeyOut, ValIn, ValOut>} txnOrDb
   * @param {PathIn} path
   * @param {'partition' | NativeValue} [layer]
   * @returns {Promise<Directory<KeyIn, KeyOut, ValIn, ValOut>>}
   */
   ...
   * @param {PathIn} _path
   * @param {'partition' | NativeValue} layer
   * @param {Buffer} [reqPrefix]
   * @param {boolean} allowCreate
   * @param {boolean} allowOpen
   * @returns {Promise<Directory<KeyIn, KeyOut, ValIn, ValOut>>}
   */
  async _createOrOpenInternal(txnOrDb, _path, layer = emptyBuffer, reqPrefix, allowCreate = true, allowOpen = true) {
    ...
    return doTxn(txnOrDb, async txn => {
      ...
      if (existingNode.exists()) {
        ...
        if (existingNode.isInPartition()) {
          ...
          return /** @type {Directory<any, any, any, any>} */(existingNode.getContentsSync(this))._directoryLayer._createOrOpenInternal(
            txn,
            subpath,
            layer,
            reqPrefix,
        ...
        if (layerBuf.length && !/** @type {Buffer} */(existingNode.layer).equals(layerBuf)) {
          throw new DirectoryError('The directory was created with an incompatible layer.')
        }

        return /** @type {Directory<any, any, any, any>} */(existingNode.getContentsSync(this, keyXf, valueXf))
      }
   ...
   * @returns {Promise<Directory<KeyIn, KeyOut, ValIn, ValOut>>}
   */
  open(txnOrDb, path, layer = emptyBuffer) {
   ...
   * @returns {Promise<Directory<KeyIn, KeyOut, ValIn, ValOut>>}
   */
  create(txnOrDb, path, layer = emptyBuffer, prefix) {
   ...
   * @template KeyIn, KeyOut, ValIn, ValOut
   * @param {DbOrTxn<KeyIn, KeyOut, ValIn, ValOut>} txnOrDb
   * @param {PathIn} _oldPath
   * @param {PathIn} _newPath
   * @returns {Promise<Directory<KeyIn, KeyOut, ValIn, ValOut>>}
   */
  move(txnOrDb, _oldPath, _newPath) {
    ...
    return doTxn(txnOrDb, async txn => {
      ...
      if (oldNode.isInPartition() || newNode.isInPartition()) {
        ...
        return /** @type {Directory<any, any, any, any>} */(newNode.getContentsSync(this)).move(txn, oldNode.getPartitionSubpath(), newNode.getPartitionSubpath())
      }
      ...
      // Ok actually move.
      const oldPrefix = this._getPrefixForNode(/** @type {NodeSubspace} */(oldNode.subspace))
      txn.at(/** @type {NodeSubspace} */(parentNode.subspace)).set([SUBDIRS_KEY, newPath[newPath.length - 1]], oldPrefix)
      ...
      return this._contentsOfNode(/** @type {NodeSubspace} */(oldNode.subspace), newPath, /** @type {Buffer} */(oldNode.layer), keyXf, valueXf)
    })
  }
   ...
   * @param {TxnAny | DbAny} txnOrDb
   * @param {PathIn} _path
   * @param {boolean} failOnNonexistent
   * @returns {Promise<boolean>}
   */
  _removeInternal(txnOrDb, _path, failOnNonexistent) {
    ...
    return doTxn(txnOrDb, async txn => {
      ...
      if (node.isInPartition()) {
        return /** @type {Directory<any, any, any, any>} */(node.getContentsSync(this))
          ._directoryLayer
          ._removeInternal(txn, node.getPartitionSubpath(), failOnNonexistent)
      }

      await this._removeRecursive(txn, /** @type {NodeSubspace} */(node.subspace))
  ...
  async* list(txn, _path) {
    ...
    if (node.isInPartition(true)) {
      yield* /** @type { Directory<any, any, any, any>} */(node.getContentsSync(this))
        .list(txn, node.getPartitionSubpath())
    } else {
      for await (const [name] of this._subdirNamesAndNodes(txn, /** @type {NodeSubspace} */(node.subspace))) {
        yield name
      }
    }
  }
  ...
  async exists(txnOrDb, _path = []) {
    return doTxn(txnOrDb, async txn => {
      ...
      if (node.isInPartition()) {
        return /** @type {Directory<any, any, any, any>} */(node.getContentsSync(this)).exists(txn, node.getPartitionSubpath())
      }
   ...
   * @param {TxnAny} txn
   * @param {Buffer} key
   * @returns {Promise<NodeSubspace | undefined>}
   */
  async _nodeContainingKey(txn, key) {
    ...
    if (prev) {
      const [k] = prev
      const previousPrefix = /** @type {Buffer} */(k[0])
  ...
  /**
   * @param {NodeSubspace} node
   * @returns {Buffer}
   */
  _getPrefixForNode(node) {
    ...
    return /** @type {Buffer} */(this._nodeSubspace._bakedKeyXf.unpack(node.prefix)[0])
  }
   ...
   */
  _contentSubspaceForNodeWithXF(node, keyXf, valueXf) {
   ...
   * @returns {Subspace}
   */
  _contentSubspaceForNode(node) {
    return this._contentSubspaceForNodeWithXF(node, identity, identity)
  }
   ...
   */
  async _find(txn, path) {
    let node = new Node(this._rootNode, [], path)

    for (let i = 0; i < path.length; i++) {
      const ref = await txn.at(/** @type {NodeSubspace} */(node.subspace)).get([SUBDIRS_KEY, path[i]])
      node = new Node(
   ...
   * @returns {Promise<Node>}
   */
  async _findWithMeta(txn, targetPath) {
  ...
  /**
   * @template KeyIn, KeyOut, ValIn, ValOut
   * @param {NodeSubspace} nodeSubspace
   * @param {Path} path
   * @param {NativeValue} layer
   * @param {Transformer<KeyIn, KeyOut>} keyXf
   * @param {Transformer<ValIn, ValOut>} valueXf
   * @returns {Directory<KeyIn, KeyOut, ValIn, ValOut>}
   */
  _contentsOfNode(nodeSubspace, path, layer = emptyBuffer, keyXf, valueXf) {
    // This is some black magic. We have a reference to the node's subspace, but
    // what we really want is the prefix. So we want to do the inverse to
    // this._nodeSubspace.at(...).
  ...
  async* _subdirNamesAndNodes(txn, node) {
    for await (const [key, prefix] of txn.at(node).getRangeStartsWith(SUBDIRS_KEY)) {
      yield /** @type {[Buffer, NodeSubspace]} */([key[1], this._nodeWithPrefix(prefix)])
    }
  }
  ...
  async _removeFromParent(txn, path) {
    ...
    txn.at(/** @type {NodeSubspace} */(parent.subspace)).clear([SUBDIRS_KEY, path[path.length - 1]])
  }
