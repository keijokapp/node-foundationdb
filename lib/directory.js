...
export class HighContentionAllocator {
  ...
  constructor(subspace) {
    /** @type {Subspace<TupleIn, TupleItem[], bigint, bigint>} */
  ...
  async allocate(_tn) {
    ...
    for (;;) {
      ...
      for (;;) {
        ...
        if (/** @type {any[]} */(latestCounter).length > 0 && /** @type {any[]} */(latestCounter)[0] > start) {
          break
        }
  ...
  constructor(parentDirectoryLayer, path, contentSubspace, isPartition, layer) {
    ...
    if (isPartition) {
      ...
      const directoryLayer = new DirectoryLayer({
        nodeSubspace: contentSubspace.atRaw(DEFAULT_NODE_PREFIX).withKeyEncoding(tuple).withValueEncoding(identity),
        contentSubspace,
      })
...
export class DirectoryLayer {
  ...
  constructor(opts = {}) {
    ...
    this._nodeSubspace = opts.nodeSubspace != null
      ...
      : new Subspace(opts.nodePrefix ?? DEFAULT_NODE_PREFIX, tuple, identity)
    ...
    this._contentSubspace = opts.contentSubspace != null
      ...
      : new Subspace(opts.contentPrefix ?? emptyBuffer, tuple, identity)
