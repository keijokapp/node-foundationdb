 ...
 * @import { DirectoryLayerOpts, NativeValue, Transformer } from './types'
 */
...
export class DirectoryLayer {
  ...
  move(txnOrDb, _oldPath, _newPath) {
    ...
    return doTxn(txnOrDb, async txn => {
      ...
      // Ok actually move.
      const oldPrefix = this.getPrefixForNode(/** @type {NodeSubspace} */(oldNode.subspace))
   ...
   * @returns {Promise<Node>}
   */
  async findWithMeta(txn, targetPath) {
