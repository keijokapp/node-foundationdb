...
export class HighContentionAllocator {
  ...
  async allocate(_tn) {
    ...
    for (;;) {
      let [[start], count] = /** @type {[[number], bigint]} */((await counters.snapshot().getRangeAllStartsWith(undefined, { limit: 1, reverse: true }))[0])
        ?? [[0], 0n]
      ...
      for (;;) {
        ...
        await synchronized(_tn, async () => {
          ...
          count = /** @type {bigint} */(await counters.snapshot().get(start))
      ...
      for (;;) {
        ...
        await synchronized(_tn, async () => {
          [latestCounter] = (await counters.snapshot().getRangeAllStartsWith(undefined, { limit: 1, reverse: true })).map(([[start]]) => start)
        ...
        if (latestCounter != null && latestCounter > start) {
          break
        }
