...
import {networkOptionData} from './opts.g'
...
export const encoders = {
  int32BE: /** @type {import('foundationdb').Transformer<number, number>} */({
    pack(num) {
  ...
  json: /** @type {import('foundationdb').Transformer<any, any>} */({
    pack(obj) { return JSON.stringify(obj) },
    unpack(buf) { return JSON.parse(buf.toString('utf8')) }
  }),

  string: /** @type {import('foundationdb').Transformer<string, string>} */({
    pack(str) { return Buffer.from(str, 'utf8') },
    unpack(buf) { return buf.toString('utf8') }
  }),

  buf: /** @type {import('foundationdb').Transformer<Buffer, Buffer>} */({
    pack: id,
    unpack: id
  }),

  // TODO: Move this into a separate library
  tuple: /** @type {import('foundationdb').Transformer<import('foundationdb').TupleItem[], import('foundationdb').TupleItem[]>} */(tuple),
}

/**
 * Can only be called before open() or openSync().
 *
 * @param {import('foundationdb').NetworkOptions} netOpts
 */
 ...
 * @param {import('foundationdb').DatabaseOptions}  [dbOpts]
 */
