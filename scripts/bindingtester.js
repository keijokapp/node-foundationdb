...
//
// 'node': Tester('node', '/home/seph/src/node-foundationdb/scripts/bindingtester.js', 53, 500, 730, types=ALL_TYPES),
//
...
import { concat2, emptyBuffer, startsWith } from '../lib/util.js'
import { packPrefixedVersionstamp } from '../lib/versionstamp.js'

/**
 * @import { WriteStream } from 'node:fs'
 * @import { ChalkInstance } from 'chalk'
 * @import { TupleItem } from 'fdb-tuple'
 * @import { StreamingMode, Transaction, TransactionOptions, Transformer } from '../lib/index.js'
 */
...
/** @type {Transformer<TupleItem | TupleItem[], TupleItem[]>} */
const tupleStrict = {
  ...tuple,
  name: 'tuple strict',
  unpack(val) {
 ...
 * @returns {{ run(instrBuf: Buffer, log?: WriteStream): Promise<void> }}
 */
const makeMachine = (db, initialName) => {
  /** @type {{ instrId: number, data: any }[]} */
  const stack = []
  ...
  const popValue = async () => {
    ...
    return /** @type {{ data: any }} */(stack.pop()).data
  }
  ...
  const operations = {
    ...
    async ATOMIC_OP(oper) {
      const codeStr = /** @type {keyof typeof MutationType} */(toUpperCamelCase(await popStr()))
...
process.on('unhandledRejection', /** @param {any} e */e => {
  console.log(chalk.redBright('✖'), 'Unhandled error in binding tester:\n', e.message, 'code', e.code, e.stack)
